...
// Retrato do mapa anterior, LINHA INTEIRA por coluna. Duas coisas dependem dele,
// e a segunda e a que faz a diferenca entre firmar ser reversivel ou nao:
//
//   1. a escolha ja feita na coluna "Firmar?" — regerar o mapa depois de uma
//      mudanca na planilha nao pode zerar a revisao feita coluna a coluna;
//
//   2. A FORMULA ARQUIVADA DAS COLUNAS JA FIRMADAS. Coluna firmada nao tem mais
//      formula: o mfVarrer_ nao a enxerga (qtd === 0), ela sumia da lista e o
//      mfEscreverMapa_ — que faz aba.clear() antes de escrever — levava junto o
//      UNICO arquivo da formula que existia. Dali em diante o
//      restaurarFormulasPaginaTransferencia() nao tinha o que repor: a logica da
//      coluna estava perdida de vez, e o unico caminho de volta era reescrever a
//      formula a mao. Regerar o mapa e uma operacao de leitura na cabeca de quem
//      roda — nao pode apagar a unica copia de nada.
function mfMapaAnterior_(ss) {
  ...
  dados.forEach(function (linha) {
    ...
    if (letra) anterior.set(letra, linha);
  });
  return anterior;
}
...
function mfFirmarAnterior_(ss) {
  ...
  mfMapaAnterior_(ss).forEach(function (linha, letra) {
    escolhas.set(letra, mfTxt_(linha[MF_C_FIRMAR]));
  });
  return escolhas;
}
...
function mfEscreverMapa_(ss, resultado, anterior) {
  ...
  const linhas = resultado.colunas.map(function (c) {
    const anteriorDaColuna = anterior.get(c.letra);
    const escolha = anteriorDaColuna ? mfTxt_(anteriorDaColuna[MF_C_FIRMAR]) : "";
  ...
  // AS COLUNAS QUE A VARREDURA NAO VE MAIS, porque ja estao firmadas. Voltam
  // exatamente como estavam — a formula arquivada e o "Firmar?" intactos — so com
  // "Nº de fórmulas" em 0, que e o que diz na propria aba que aquela coluna hoje e
  // valor. Sem isto, regerar o mapa uma vez depois de firmar apagava a formula.
  const vistas = {};
  resultado.colunas.forEach(function (c) { vistas[c.letra] = true; });

  const preservadas = [];
  anterior.forEach(function (linhaAnterior, letra) {
    if (vistas[letra]) return;

    const a1 = mfTxt_(linhaAnterior[MF_C_FORMULA]);
    const r1c1 = mfTxt_(linhaAnterior[MF_C_R1C1]);
    if (!a1 && !r1c1) return;   // nada arquivado: nao ha o que preservar

    const registro = linhaAnterior.slice(0, MF_MAPA_HEADERS.length);
    registro[MF_C_QTD] = 0;
    // O apostrofo some na leitura (e marcador de texto, nao conteudo) e precisa
    // voltar na escrita — sem ele a planilha AVALIA a formula arquivada.
    registro[MF_C_FORMULA] = a1 ? "'" + a1 : "";
    registro[MF_C_R1C1] = r1c1 ? "'" + r1c1 : "";
    preservadas.push(registro);
    linhas.push(registro);
  });

  if (preservadas.length > 0) {
    console.log("Coluna(s) preservada(s) no mapa por já estarem firmadas (sem fórmula na " +
                "página, fórmula arquivada mantida): " +
                preservadas.map(function (r) { return mfTxt_(r[MF_C_LETRA]); }).join(", ") + ".");
  }
...
function mapearFormulasPaginaTransferencia(opcoes) {
  ...
  const anterior = mfMapaAnterior_(ss);
  const resultado = mfVarrer_(aba);

  if (resultado.colunas.length === 0) {
    // Sem escrever nada: numa pagina inteiramente firmada nao ha formula para
    // varrer, e reescrever o mapa aqui seria apaga-lo.
    console.log("Nenhuma fórmula encontrada em '" + nomeAba + "'. Nada a mapear " +
                "(mapa anterior mantido como está).");
    return resultado;
  }

  mfEscreverMapa_(ss, resultado, anterior);
